...
if st.session_state.filtered_selection is not None:
    ...
    if st.session_state.filtered_selection.empty:
        ...
        # Basic performance metrics for the filtered selection
        try:
            numeric_cols = ['3 Years Annualised (%)', 'Investment Management Fee(%)', '3 Year Standard Deviation', '3 Year Sharpe Ratio']
            available_cols = [col for col in numeric_cols if col in st.session_state.filtered_selection.columns]
            
            if available_cols:
                summary_stats = st.session_state.filtered_selection[available_cols].describe()
                
                # Display key metrics
                col1, col2, col3, col4 = st.columns(4)
                
                with col1:
                    if '3 Years Annualised (%)' in available_cols:
                        avg_return = st.session_state.filtered_selection['3 Years Annualised (%)'].mean()
                        st.metric("Average Return", f"{avg_return:.2f}%")
                
                with col2:
                    if 'Investment Management Fee(%)' in available_cols:
                        avg_fee = st.session_state.filtered_selection['Investment Management Fee(%)'].mean()
                        st.metric("Average Fee", f"{avg_fee:.2f}%")
                
                with col3:
                    if '3 Year Standard Deviation' in available_cols:
                        avg_risk = st.session_state.filtered_selection['3 Year Standard Deviation'].mean()
                        st.metric("Average Risk", f"{avg_risk:.2f}")
                
                with col4:
                    if '3 Year Sharpe Ratio' in available_cols:
                        avg_sharpe = st.session_state.filtered_selection['3 Year Sharpe Ratio'].mean()
                        st.metric("Average Sharpe", f"{avg_sharpe:.2f}")
                
                # Show distribution by category
                if 'Morningstar Category' in st.session_state.filtered_selection.columns:
                    st.markdown("**Distribution by Category:**")
                    category_counts = st.session_state.filtered_selection['Morningstar Category'].value_counts()
                    st.dataframe(category_counts.head(10), use_container_width=True)
            else:
                st.info("Performance metrics are not available for the filtered selection.")
                
        except Exception as e:
            st.error(f"Error calculating selection performance: {str(e)}")
            st.info("Performance summary couldn't be calculated, but your filtered data is still available below.")
